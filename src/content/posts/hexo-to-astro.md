...
3. **不需要手动构建**：构建过程是 Cloudflare Pages 自动执行的，只需要上传就完事。同样也解决了Hexo速度慢和需要备份源文件的问题。（同样类似 VitePress）
...
换了 Astro 最能感知到的变化就是主题更美观了，同时编写文章也更方便了。

这是一个比较新的框架，目的是减少JS脚本，提高加载速度。目前用下来除了主题没有什么明显的差异。

相比于 Hexo 和 Hugo，个人认为 Astro 更适合搭博客。
